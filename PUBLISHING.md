...
| 項目 | 值 |
| --- | --- |
| GitHub repo | `doggy8088/opencc-py` |
| PyPI distribution | `opencc-py-tw2` |
| Python import package | `opencc_py` |
| CLI command | `opencc-py` |
| 發布 workflow | `.github/workflows/publish.yml` |
| GitHub environment | `pypi` |

> `opencc-py` 這個 PyPI distribution name 已被既有專案使用，所以本專案發布名稱是 `opencc-py-tw2`。
...
建議用 venv，避免 macOS/Homebrew Python 的 PEP 668 限制。
...
- Project name 是 `opencc-py-tw2`，不是 `opencc-py`。
- Owner 是 `doggy8088`。
- Repository name 只填 `opencc-py`，不要填完整 URL。
- Workflow name 是 `publish.yml`，不是 `.github/workflows/publish.yml`。
- Environment name 是 `pypi`。
...
gh release create v0.1.0 \
  ...
  --target main \
  --title "v0.1.0" \
  --notes "Initial pure Python OpenCC port based on the C# implementation."
```
...
成功時，`Publish to PyPI` step 會完成，PyPI 會出現 `opencc-py-tw2` 的新版本。
